...
const StudyManager = ({ prolificId, globalTimeSpent }) => {
  ...
  const handleAdjustmentComplete = async (finalRect) => {
    ...
    // Get the round we are currently adjusting
    const currentRound = allRoundsData[currentRoundIndex];
    
    // Prepare data payload
    const payload = {
      prolificId,
      roundId: currentRound.config.id,
      imageSrc: currentRound.config.imageSrc,
    ...
    try {
      ...
      console.log('Round data sent successfully');
    } catch (error) {
